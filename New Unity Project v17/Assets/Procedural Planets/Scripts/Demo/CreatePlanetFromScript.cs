...
public class CreatePlanetFromScript : MonoBehaviour {
    ...
    public void CreateRandomPlanet()
    {
        ...
        // Use PlanetManager.CreatePlanet method to create a new planet - when only a position is included as an argument it will create a random solid planet
        string _solidPlanetBlueprint = PlanetManager.Instance.listSolidPlanetBlueprints[Random.Range(0, PlanetManager.Instance.listSolidPlanetBlueprints.Count)].name;
        _planet = PlanetManager.CreatePlanet(Vector3.zero, -1, _solidPlanetBlueprint);
